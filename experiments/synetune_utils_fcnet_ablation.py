...
class SyneTuneBenchmark(BENCHMARK):
    ...
    def __init__(self, config_space: Dict, seed: int):
        super().__init__()
        self.original_config_space = config_space  # Keep original for reference
        self.metrics = []
        self.seed = seed
        self.random_state = np.random.RandomState(self.seed)
        self.benchmark_name = "SyneTune/fcnet-slice-no-context"  # TODO: revert this
        self.model_name = "gemini-2.0-flash"
        self.problem_id = "fcnet_slice"
        self.range_parameter_keys = []

        # Remap config_space to x0, x1, x2... format
        self.config_space = {
            f"x{i}": domain for i, (name, domain) in enumerate(config_space.items())
        }

        # Create hp_ranges with remapped keys
    ...
    def _sample_random(self, **kwargs) -> Dict:
        ...
        # Remap to x0, x1, x2... format
        random_samples = {
            f"x{i}": v.sample(random_state=self.random_state)
            if isinstance(v, Domain)
            else v
            for i, (k, v) in enumerate(self.config_space.items())
        }
        print(f"random_samples: {random_samples}")
        return random_samples
    ...
    def is_valid_candidate(self, candidate) -> bool:
        ...
        :param candidate: Dictionary containing the hyperparameter values (with x0, x1, x2... keys)
        :return: True if the point is valid, False otherwise
        """
        print(f"Candidate {candidate}")
        print(f"search space {self.config_space}")

        # Map x0, x1, x2... to the corresponding domain in config_space
        config_space_list = list(self.config_space.items())
        for param_key, param_val in candidate.items():
            # Extract index from x0, x1, x2... format
            if param_key.startswith("x"):
                idx = int(param_key[1:])
                if idx < len(config_space_list):
                    name, constraint = config_space_list[idx]
                else:
                    print(f"Invalid index {idx} for parameter {param_key}")
                    return False
            else:
                # Fallback to original behavior if not in x format
                constraint = self.config_space.get(param_key)
                if constraint is None:
                    print(f"Unknown parameter {param_key}")
                    return False

            print(f"Constraints: {constraint}")
            if isinstance(constraint, Domain):
...
class LLMSearcher(SingleObjectiveBaseSearcher):
        ...
        **surrogate_kwargs,
        ...
        self.mohollm_config = {
            ...
            "space_partitioning_settings": {
                ...
                # "ablation": "exploration",  # TODO: Change this for ablation to [ exploration,exploitation, uniform_region_sampling, ucb1]
        ...
        if config is None:
            if self.config_queue:
                ...
                if type(config) is list:
                    ...
                    for c in config:
                        remapped = {
                            hp_names[i]: c[f"x{i}"] for i in range(len(hp_names))
                        }
                        config_mapped.append(remapped)

                    # If we get more than one config, we need to store them for syne tune and return them one by one
                    self.config_queue = config_mapped[1:]
                    config = config_mapped[0]
                else:
                    # Also remap single config
                    hp_names = [
                        "hp_activation_fn_1",
                        "hp_activation_fn_2",
                        "hp_batch_size",
                    ...
                    config = {
                        hp_names[i]: config[f"x{i}"] for i in range(len(hp_names))
                    }
    ...
    def on_trial_complete(
        ...
        # Remap config to x0, x1, x2... format before updating statistics
        hp_names = list(config.keys())
        remapped_config = {f"x{i}": config[hp_names[i]] for i in range(len(hp_names))}

        # Here we have to manually update the statistics of the mohollm right now.
        self.mohollm.update_statistics(
            sel_candidate_point=remapped_config, sel_candidate_eval=candidate_eval
        )
