...
from plot_settings import get_color, LABEL_MAP_HV, HV_REFERENCE_POINTS
...
def main():
    ...
    for benchmark in benchmarks:
        # find observed fvals files
        search_path = os.path.join(base_data_path, benchmark, "**", "*.csv")
        file_names = glob.glob(search_path, recursive=True)
        observed_fvals_files = [f for f in file_names if "observed_fvals" in f]
        if not observed_fvals_files:
            continue
        method_files = group_data_by_method(observed_fvals_files, filter)
            ...
            for file in files:
                try:
                    ...
                    if "configs" in df.columns:
                        df = df.drop(columns=["configs"])
                    dfs.append(df)
                except Exception as e:
                    print(f"Error reading observed fvals file {file}: {e}")
            if dfs:
                method_dfs[method] = dfs
        ...
        # determine objective columns from first available df
        first_method = next(iter(method_dfs))
        first_df = method_dfs[first_method][0]
        obj_cols = [c for c in first_df.columns if c != "configs"]

        # compute HV per seed per method using raw observed fvals
        hv_data = {}
        for method, dfs in method_dfs.items():
            seed_hvs = []
            for df in dfs:
                # keep only objective columns (raw values)
                fvals = df[obj_cols].copy()
                # choose reference point: use HV_REFERENCE_POINTS if defined for this benchmark
                ref = None
                try:
                    ref = HV_REFERENCE_POINTS.get(benchmark)
                except Exception:
                    ref = None
                if ref is None:
                    ref = [1.0] * len(obj_cols)
                hv_series = convert_data_to_hv_over_time(fvals, reference_point=ref)
                seed_hvs.append(hv_series)
            if seed_hvs:
                hv_data[method] = seed_hvs

        # get per-benchmark cost and time dicts (method -> list[df])
        bench_cost = aggregated_by_benchmark.get(benchmark, {}).get(
            "cost_per_request", {}
        )
        bench_time = aggregated_by_benchmark.get(benchmark, {}).get(
            "time_taken_per_trials", {}
        )

        # create plots
        hv_plot_dir = os.path.join(output_path, "hv_vs")
        os.makedirs(hv_plot_dir, exist_ok=True)
        # TODO: The plot is still broken
        create_hv_vs_cost_plot(
            hv_data, bench_cost, hv_plot_dir, f"{filename}_{benchmark}", trials
        )
        # TODO: The plot is still broken
        create_hv_vs_time_plot(
            hv_data, bench_time, hv_plot_dir, f"{filename}_{benchmark}", trials
        )
    ...
    create_grouped_rejection_rate_plot(filtering_data, filtering_plot_dir, filename)
...
def create_grouped_rejection_rate_plot(filtering_data, path, filename):
    """Creates a 1x2 plot comparing rejection rates per trial for MOHOLLM vs LLM.

    For each group we average across seeds and methods that belong to the group.
    """
    # Prepare groups and collect per-seed series for each rejection type
    groups = {
        "MOHOLLM": {"dup": [], "obs": [], "reg": []},
        "LLM": {"dup": [], "obs": [], "reg": []},
    }

    # Collect per-seed series for each rejection type
    for method, seed_dfs in filtering_data.items():
        key = "MOHOLLM" if "mohollm" in method.lower() else "LLM"
        for df in seed_dfs:
            denom = df["num_proposed_candidates"].replace({0: np.nan})
            dup = (df["rejected_duplicates"] / denom * 100).fillna(0)
            obs = (df["rejected_observed"] / denom * 100).fillna(0)
            reg = (df["rejected_region"] / denom * 100).fillna(0)
            groups[key]["dup"].append(dup)
            groups[key]["obs"].append(obs)
            # Only MOHOLLM uses a region-based rejection; do not collect
            # 'Out of Region' series for the LLM group so the red line
            # does not appear for LLM (it would be misleading).
            if key == "MOHOLLM":
                groups[key]["reg"].append(reg)

    # Plot
    fig, axes = plt.subplots(1, 2, figsize=(5, 3), sharey=True)
    colors = {"dup": "#ff7f0e", "obs": "#2ca02c", "reg": "#d62728"}
    for ax_idx, (grp_name, series_dict) in enumerate(groups.items()):
        ax = axes[ax_idx]
        for key, label in [
            ("dup", "Duplicates"),
            ("obs", "Already Observed"),
            ("reg", "Out of Region"),
        ]:
            s_list = series_dict[key]
            if not s_list:
                continue
            max_trial = max(s.index.max() for s in s_list if not s.empty)
            common_index = pd.RangeIndex(start=0, stop=max_trial + 1)
            aligned = [s.reindex(common_index, fill_value=0) for s in s_list]
            arr = np.array([s.values for s in aligned])
            mean = np.mean(arr, axis=0)
            n = arr.shape[0]
            se = (
                1.96 * np.std(arr, axis=0, ddof=1) / np.sqrt(n)
                if n > 1
                else np.zeros_like(mean)
            )

            ax.plot(common_index, mean, label=label, color=colors[key], lw=2)
            if np.any(se > 0):
                ax.fill_between(
                    common_index, mean - se, mean + se, color=colors[key], alpha=0.2
                )
    ...
    # Create a single, shared legend for the figure (one legend only)
    legend_handles = [
        Line2D([0], [0], color=colors["dup"], lw=2),
        Line2D([0], [0], color=colors["obs"], lw=2),
        Line2D([0], [0], color=colors["reg"], lw=2),
    ]
    legend_labels = ["Duplicates", "Already Observed", "Out of Region"]
    # Place legend centered below subplots
    fig.legend(
        legend_handles,
        legend_labels,
        loc="lower center",
    ...
    for ext in ["png", "svg", "pdf"]:
        outpath = os.path.join(path, f"{filename}_grouped_rejection_rates.{ext}")
...
def create_hv_vs_cost_plot(hv_data, cost_data, path, filename, trials=None):
    """Plot Hypervolume vs Cumulative Cost for each method.

    hv_data: dict method -> list of 1D hv arrays (per-seed)
    cost_data: dict method -> list of dataframes with 'trial' and 'total_cost'
    """
    fig, ax = plt.subplots(figsize=DEFAULT_FIGSIZE)
    methods = sorted(set(list(hv_data.keys()) + list(cost_data.keys())))

    for i, method in enumerate(methods):
        if (
            method != "MOHOLLM (Gemini 2.0 Flash)"
            or method != "mohollm (Gemini 2.0 Flash)"
            or method != "MOHOLLM (Gemini 2.0 Flash) (Context)"
        ):
            continue
        color = get_color(method, i)
        hv_seeds = hv_data.get(method, [])
        cost_seeds = cost_data.get(method, []) if cost_data else []
        if not hv_seeds:
            continue

        # build cumulative cost arrays for each seed aligning to hv length
        cost_curves = []
        for idx, hv_series in enumerate(hv_seeds):
            # try to find matching cost df by index; if missing, skip
            cost_df = None
            if idx < len(cost_seeds):
                cost_df = cost_seeds[idx]
            if (
                cost_df is None
                or "trial" not in cost_df.columns
                or "total_cost" not in cost_df.columns
            ):
                # fallback: create zeros
                cost_curves.append(np.zeros_like(hv_series))
            else:
                g = cost_df.groupby("trial", as_index=False)["total_cost"].sum()
                if trials is not None:
                    g = g[g["trial"] < trials]
                cum = g["total_cost"].cumsum().values
                # align/truncate/pad to hv length
                L = len(hv_series)
                if len(cum) >= L:
                    cost_curves.append(cum[:L])
                else:
                    cost_curves.append(
                        np.pad(
                            cum,
                            (0, L - len(cum)),
                            constant_values=cum[-1] if len(cum) > 0 else 0.0,
                        )
                    )

        # align hv seeds to same length
        hv_arr = _align_and_pad_series(
            [np.array(s) for s in hv_seeds],
            pad_value=hv_seeds[0][-1] if len(hv_seeds[0]) > 0 else 0.0,
        )
        cost_arr = _align_and_pad_series(cost_curves, pad_value=0.0)

        # compute mean and std error across seeds but along trials dimension
        mean_cost = np.mean(cost_arr, axis=0)
        mean_hv = np.mean(hv_arr, axis=0)
        if cost_arr.shape[0] > 1:
            se_hv = 1.96 * np.std(hv_arr, axis=0, ddof=1) / np.sqrt(cost_arr.shape[0])
        else:
            se_hv = np.zeros_like(mean_hv)

        # sort by increasing cost to make a monotonic X for plotting (use mean_cost ordering)
        ...
        if np.any(se_hv > 0):
            ax.fill_between(
                mean_cost[order],
                mean_hv[order] - se_hv[order],
                mean_hv[order] + se_hv[order],
                color=color,
                alpha=0.2,
            )

    ax.set_xlabel(r"Cumulative Cost (\$)")
    ax.set_ylabel("Hypervolume")
    ax.set_title("Hypervolume vs Cumulative Cost")
    ax.grid(True, linestyle="--", alpha=0.6)
    apply_axis_style(ax)
    ax.legend(loc="lower right")
    plt.tight_layout()
    for ext in ["png", "svg", "pdf"]:
        out = os.path.join(path, f"{filename}_hv_vs_cost.{ext}")
        os.makedirs(os.path.dirname(out), exist_ok=True)
        plt.savefig(out, dpi=300, bbox_inches="tight")
    plt.close()


def create_hv_vs_time_plot(hv_data, time_data, path, filename, trials=None):
    """Plot Hypervolume vs Cumulative Time for each method.

    time_data: dict method -> list of dataframes with 'trial' and 'trial_total_time'
    """
    fig, ax = plt.subplots(figsize=DEFAULT_FIGSIZE)
    methods = sorted(set(list(hv_data.keys()) + list(time_data.keys())))

    for i, method in enumerate(methods):
        color = get_color(method, i)
        hv_seeds = hv_data.get(method, [])
        time_seeds = time_data.get(method, []) if time_data else []
        if not hv_seeds:
            continue
        ...
        for idx, hv_series in enumerate(hv_seeds):
            time_df = None
            if idx < len(time_seeds):
                time_df = time_seeds[idx]
            if (
                time_df is None
                or "trial" not in time_df.columns
                or "trial_total_time" not in time_df.columns
            ):
                time_curves.append(np.zeros_like(hv_series))
            else:
                if "trial" in time_df.columns:
                    g = time_df.groupby("trial", as_index=False)[
                        "trial_total_time"
                    ].sum()
                else:
                    g = (
                        time_df[["trial_total_time"]]
                        .reset_index()
                        .rename(columns={"index": "trial"})
                    )
                if trials is not None:
                    g = g[g["trial"] < trials]
                cum = g["trial_total_time"].cumsum().values
                L = len(hv_series)
                if len(cum) >= L:
                    time_curves.append(cum[:L])
                else:
                    time_curves.append(
                        np.pad(
                            cum,
        ...
        time_arr = _align_and_pad_series(time_curves, pad_value=0.0)

        mean_time = np.mean(time_arr, axis=0)
        mean_hv = np.mean(hv_arr, axis=0)
        if time_arr.shape[0] > 1:
            se_hv = 1.96 * np.std(hv_arr, axis=0, ddof=1) / np.sqrt(time_arr.shape[0])
        else:
            se_hv = np.zeros_like(mean_hv)

        order = np.argsort(mean_time)
        ax.plot(
            mean_time[order],
            mean_hv[order],
            label=LABEL_MAP_HV.get(method, method),
    ...
    ax.set_xlabel(r"Cumulative Time (s)")
    ax.set_ylabel("Hypervolume")
    ax.set_title("Hypervolume vs Cumulative Time")
    ax.grid(True, linestyle="--", alpha=0.6)
    apply_axis_style(ax)
    ax.legend(loc="lower right")
    plt.tight_layout()
    for ext in ["png", "svg", "pdf"]:
        out = os.path.join(path, f"{filename}_hv_vs_time.{ext}")
        os.makedirs(os.path.dirname(out), exist_ok=True)
        plt.savefig(out, dpi=300, bbox_inches="tight")
    plt.close()
...
def create_summary_table(aggregated_by_benchmark, trials, path):
    ...
    for method, vals in method_rows.items():
        ...
        summary = {"Benchmark": "Average", "Method": method}
    ...
    # Split into MOHOLLM and LLM files
    moh_rows = [r for r in rows if "mohollm" in r["Method"].lower()]
    llm_rows = [r for r in rows if "mohollm" not in r["Method"].lower()]
    # append summaries
    moh_rows.extend([r for r in summary_rows if "mohollm" in r["Method"].lower()])
    llm_rows.extend([r for r in summary_rows if "mohollm" not in r["Method"].lower()])
    ...
    write_md(moh_rows, "operational_costs_mohollm.md", "Operational Costs - MOHOLLM")
    write_md(llm_rows, "operational_costs_llm.md", "Operational Costs - LLM")
    ...
    # write for both method groups
    write_metric_files(moh_rows, "mohollm")
    write_metric_files(llm_rows, "llm")

    # Finally, create a compact table with a single row per method group
    # (MOHOLLM vs LLM) containing the average values across benchmarks.
    method_groups = {
        "MOHOLLM": [r for r in rows if "mohollm" in r["Method"].lower()],
        "LLM": [r for r in rows if "mohollm" not in r["Method"].lower()],
    }
    ...
    for grp_name, grp_rows in method_groups.items():
        if not grp_rows:
            # empty group -> zeros
    ...
    # Prepare dataframe for output and format columns like other outputs
    # Recompute group aggregates (mean and std) directly from numeric *_mean / *_std
    # fields created per-benchmark so we do not parse formatted strings.
    agg_table = []
    # mapping from display key -> (mean_col_name, std_col_name)
    numeric_map = {
        "Avg. Time / Trial (s)": (
            "Avg. Time / Trial (s)_mean",
            "Avg. Time / Trial (s)_std",
        ...
        "Avg. Total Cost ($)": ("Avg. Total Cost ($)_mean", "Avg. Total Cost ($)_std"),
        "Avg. Total Time (s)": ("Avg. Total Time (s)_mean", "Avg. Total Time (s)_std"),
    }

    for grp_name, grp_rows in method_groups.items():
        if not grp_rows:
            # empty group -> zeros
            agg_table.append({"Method": grp_name})
            continue
        dfg = pd.DataFrame(grp_rows)
        entry = {"Method": grp_name}

        def mean_std_for_key(display_key):
            # prefer numeric_map to find *_mean and *_std columns
            mean_col, std_col = numeric_map.get(display_key, (None, None))
            m = 0.0
            s = 0.0
            if mean_col and mean_col in dfg.columns:
                vals = pd.to_numeric(dfg[mean_col], errors="coerce").dropna()
                if not vals.empty:
                    m = float(vals.mean())
            if std_col and std_col in dfg.columns:
                stds = pd.to_numeric(dfg[std_col], errors="coerce").dropna()
                if not stds.empty:
                    s = float(stds.mean())
            # fallback: if numeric_map not present, try to coerce display column
            if mean_col is None:
                if display_key in dfg.columns:
                    vals = pd.to_numeric(dfg[display_key], errors="coerce").dropna()
                    if not vals.empty:
                        m = float(vals.mean())
            return (m, s)

        for k in keys:
            m, s = mean_std_for_key(k)
            entry[k] = (m, s)
        agg_table.append(entry)

    # Build a display DataFrame with formatted 'mean ± std' strings
    display_rows = []
    for ent in agg_table:
        row = {"Method": ent["Method"]}
        for k in keys:
            m, s = ent.get(k, (0.0, 0.0))
            if k == "Avg. Requests / Trial":
                row[k] = f"{int(round(m))} ± {s:.1f}"
            elif "Tokens" in k:
                row[k] = f"{int(round(m))} ± {int(round(s))}"
            elif k == "Avg. Total Cost / Trial ($)":
                # present cost per trial scaled to x10^-3
                m_s = m / 1e-3
                s_s = s / 1e-3
                row["Avg. Total Cost / Trial ($ x10^-3)"] = f"{m_s:.3f} ± {s_s:.3f}"
            elif k == "Avg. Total Cost ($)":
                row[k] = f"{m:.5f} ± {s:.5f}"
            elif "Time" in k:
                row[k] = f"{m:.2f} ± {s:.2f}"
            else:
    ...
    # decide output columns (Method + metrics where we created formatted strings)
    out_cols = ["Method"]
    out_cols.extend(
        [
            "Avg. Time / Trial (s)",
    ...
    # cost scaled
    out_cost_col = "Avg. Total Cost / Trial ($ x10^-3)"
    if any(out_cost_col in r for r in display_rows):
        out_cols.append(out_cost_col)
    if any("Avg. Total Cost ($)" in r for r in display_rows):
        out_cols.append("Avg. Total Cost ($)")
    if any("Avg. Total Time (s)" in r for r in display_rows):
        out_cols.append("Avg. Total Time (s)")

    df_display_groups = pd.DataFrame(display_rows)[out_cols]

    avg_path = os.path.join(path, "method_averages.md")
    os.makedirs(os.path.dirname(avg_path), exist_ok=True)
    with open(avg_path, "w") as f:
        f.write("# Average Values per Method Group (mean ± std)\n\n")
        if not df_display_groups.empty:
            f.write(df_display_groups.to_markdown(index=False))
        else:
            f.write("No data available")
