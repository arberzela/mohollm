...
def main():
    ...
    for benchmark in benchmarks:
        ...
        # 3. For each method and each run, compute a scalar score from the final
        # observed fvals. We use the mean of the final objectives as the scalar
        # (lower is better). Then average this scalar across runs to get one
        # value per method and benchmark.
        mean_score = {}
        for method, dfs in method_dfs.items():
            if method == "LLMKD":
                method = "HOLLM"
            ...
            for df in dfs:
                ...
                run_score = float(f1_series.min())
                run_scores.append(run_score)

            # Average across runs for this method
            if len(run_scores) > 0:
                mean_score[method] = float(np.mean(run_scores))
            else:
                # If no runs, set NaN
                mean_score[method] = float("nan")
...
def create_critical_difference_diagram(all_data, path, filename):
    # Here lower is better (we used mean of objectives). Build ranking matrix
    # where 1 is best (lowest mean).
    ranking_matrix = all_data.rank(axis=1, method="average", ascending=False)
    ...
    # plot_critical_difference expects a 2D array of scores where columns map to
    # classifiers (methods). We keep rows as benchmarks and columns as methods.
    ...
    # 4. Now, iterate through all text objects on the axes to change fonts.
    # This part is the same as before and will now work correctly.
    for text_element in ax.texts:
        text_str = text_element.get_text()

        # Check if the text is a number (a rank value)
        try:
            float(text_str)
            text_element.set_fontsize(14)
        except ValueError:
            # Otherwise, it's a method name.
            text_element.set_fontsize(20)
