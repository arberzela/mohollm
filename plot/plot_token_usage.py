...
from plot_settings import LINE_STYLES, LABEL_MAP, get_color
...
def main():
    ...
    output_path = f"./plots/token_usage/{benchmark}"
    ...
    observed_fvals_files = [
        file for file in file_names if "token_usage_per_request" in file.split("/")
    ]
    ...
    create_token_usage_plot(method_dfs, benchmark, title, output_path, filename)


def create_token_usage_plot(data, benchmark, title, path, filename):
    """
    Create publication-ready token usage over time plots.
    ...
    ax.set_ylabel(r"Total Tokens ($\times10^3$)")
    ax.set_title(title)
