...
def main():
    ...
    if n == 1:
        if args.filename == "real_world_hv_other_models_mohollm_paper":
            fig, ax = plt.subplots(1, 1, figsize=(4 * 1, 4 * 1), sharex=True)
        else:
            fig, ax = plt.subplots(1, 1, figsize=(4.2 * 1, 4 * 1), sharex=True)
        axes = [ax]
    else:
        fig, axes = plt.subplots(
            nrows, ncols, figsize=(4 * ncols, 4 * nrows), sharex=True
        )
    ...
    # Special handling for "real_world_hv_other_models_combined_paper" filename
    if args.filename == "real_world_hv_other_models_mohollm_paper":
        # Simplify legend names by removing "MOHOLLM" prefix
        simplified_labels = []
        for label in all_labels:
            # Remove "MOHOLLM " prefix if present
            if label.startswith("MOHOLLM "):
                simplified_label = label.replace("MOHOLLM ", "").strip("()")
            elif label == "MOHOLLM":
                simplified_label = "Gemini 2.0 Flash"
            else:
                simplified_label = label
            simplified_labels.append(simplified_label)
        all_labels = simplified_labels
        
        # Adjust subplot spacing to make room for right-side legend
        fig.subplots_adjust(bottom=0.1, right=0.65, wspace=0.35, hspace=0.35)
        ...
        # Default legend positioning (bottom)
        if len(all_labels) <= 4:
        ...
        def extract_param_value(label):
            """Extract numeric parameter value from label for sorting."""
            import re
            # Try to find patterns like M = 5, K = 5, m_0 = 5, or alpha_max = 1.0
            match = re.search(r'[=]\s*([\d.]+)', label)
            if match:
                return float(match.group(1))
            # If no parameter found, return infinity to put it at the end
            return float('inf')
        
        if "real_world_ablations_partitions_per_trial_paper" in args.filename:
            simplified_labels = []
            for label in all_labels:
                # Remove "MOHOLLM " prefix if present
                if label == "MOHOLLM":
                    simplified_label = r"MOHOLLM ($k = 5$)"
                else:
                    simplified_label = label
                simplified_labels.append(simplified_label)
            all_labels = simplified_labels
            # Sort by M value
            sorted_pairs = sorted(zip(all_handles, all_labels), key=lambda x: extract_param_value(x[1]))
            all_handles, all_labels = zip(*sorted_pairs)
            all_handles, all_labels = list(all_handles), list(all_labels)
        elif "real_world_ablations_candidates_per_request_paper" in args.filename:
            simplified_labels = []
            for label in all_labels:
                # Remove "MOHOLLM " prefix if present
                if label == "MOHOLLM":
                    simplified_label = r"MOHOLLM ($N = 5$)"
                else:
                    simplified_label = label
                simplified_labels.append(simplified_label)
            all_labels = simplified_labels
            ...
            all_handles, all_labels = zip(*sorted_pairs)
            all_handles, all_labels = list(all_handles), list(all_labels)
        elif "real_world_ablations_leaf_size_paper" in args.filename:
            simplified_labels = []
            for label in all_labels:
                # Remove "MOHOLLM " prefix if present
                if label == "MOHOLLM":
                    simplified_label = r"MOHOLLM ($m_0 = 5$)"
                else:
                    simplified_label = label
                simplified_labels.append(simplified_label)
            all_labels = simplified_labels
            ...
            all_handles, all_labels = zip(*sorted_pairs)
            all_handles, all_labels = list(all_handles), list(all_labels)
        elif "real_world_ablations_alpha_max_paper" in args.filename:
            simplified_labels = []
            for label in all_labels:
                # Remove "MOHOLLM " prefix if present
                if label == "MOHOLLM":
                    simplified_label = r"MOHOLLM ($\alpha_{max} = 1.0$)"
                else:
                    simplified_label = label
                simplified_labels.append(simplified_label)
            all_labels = simplified_labels
            # Sort by alpha_max value
            sorted_pairs = sorted(zip(all_handles, all_labels), key=lambda x: extract_param_value(x[1]))
            all_handles, all_labels = zip(*sorted_pairs)
            all_handles, all_labels = list(all_handles), list(all_labels)
        ...
        fig.subplots_adjust(bottom=0.3, wspace=0.35, hspace=0.35)

        # Enhanced legend with professional styling
        if n > 1:
            if "real_world_ablations_alpha_max_paper" in args.filename:
                fontsize = 10
                bbox_to_anchor = (0.4, -0.14)
            else:
                fontsize = 13
                bbox_to_anchor = (0.39, -0.2)
            ...
            ax = axes[0]
            if "surrogate_model" in args.filename:
                order = ["MOHOLLM (GP Surrogate)", "MOHOLLM (TabPFN Surrogate)", "MOHOLLM"]
            elif "candidate_sampler" in args.filename:
                order = ["LLM (Random Sampler)", "LLM", "MOHOLLM (Random Sampler)", "MOHOLLM"]
            else:
                order = ["MOHOLLM (No Context)", "MOHOLLM (Minimal)", "MOHOLLM"]
            indices = [all_labels.index(l) for l in order if l in all_labels]
            indices += [i for i, l in enumerate(all_labels) if l not in order]

            loc = "best"
            ax.legend(
                [all_handles[i] for i in indices],
