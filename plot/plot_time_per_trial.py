...
def main():
    ...
    output_path = f"./plots/time_taken/{benchmark}"
    ...
    observed_fvals_files = [
        file for file in file_names if "time_taken_per_trials" in file.split("/")
    ]
...
def create_token_usage_plot(data, benchmark, title, path, filename):
    ...
    for i, method in enumerate(methods):
        ax.plot(
            data[method][0]["trial_total_time"],
    ...
    ax.set_xlabel("Trial")
    ax.set_ylabel("Trial Total time in seconds")
    ax.set_title(title)
