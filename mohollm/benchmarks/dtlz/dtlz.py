...
try:
    ...
    raise ImportError(
        "Optional dependency 'pymoo' is required for the DTLZ benchmark.\n"
        "Install it with `pip install -e .[dev]` or `pip install pymoo` and retry."
    ) from e


logger = logging.getLogger("DTLZ")


class DTLZ(BENCHMARK):
    def __init__(self, model_name, seed, problem_id="dtlz1"):
        self.benchmark_name = "DTLZ"
        self.problem_id = problem_id
        self.n_dim = 6
        self.n_obj = 2
        self.metrics = ["f1", "f2"]
        self.problem = get_problem(problem_id, n_var=self.n_dim, n_obj=self.n_obj)
        self.seed = seed
    ...
    def evaluate_point(self, point, **kwargs) -> Dict:
        ...
        logger.debug(f"DTLZ evaluation: {f}")
        return point, {
