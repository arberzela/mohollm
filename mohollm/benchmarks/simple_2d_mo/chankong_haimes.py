...
class ChankongHaimes(BENCHMARK):
    """
    Chankong-Haimes Benchmark for Multi-Objective Optimization.
    https://en.wikipedia.org/wiki/Test_functions_for_optimization

    Note: For this benchmark we drop the constraints

    Search domain: -20 <= x, y <= 20
    """

    def __init__(self, model_name: str, seed: int):
        self.benchmark_name = "ChankongHaimes"
        self.problem_id = "chankong_haimes"
        self.metrics = ["F1", "F2"]
        self.seed = seed
    ...
    def generate_initialization(self, n_points: int, **kwargs) -> List[Dict]:
        ...
        for x in points:
            result.append(
                {"x": round(x[0] * 40 - 20, 3), "y": round(x[1] * 40 - 20, 3)}
            )
        return result
...
class TestFunction4(BENCHMARK):
    """
    Test Function 4 Benchmark for Multi-Objective Optimization.
    """
    ...
    def evaluate_point(self, point: Dict, **kwargs) -> Tuple[Dict, Dict]:
        x, y = point["x"], point["y"]
        F1 = x**2 - y  # First objective
        F2 = -0.5 * x - y - 1  # Second objective

        g1 = 6.5 - (x / 6) - y >= 0  # Constraint 1
        g2 = 7.5 - 0.5 * x - y >= 0  # Constraint 2
        g3 = 30 - 5 * x - y >= 0  # Constraint 3

        if g1 and g2 and g3:
            return point, {"F1": round(F1, 3), "F2": round(F2, 3)}
        else:
            return point, {"F1": None, "F2": None}  # Invalid point
