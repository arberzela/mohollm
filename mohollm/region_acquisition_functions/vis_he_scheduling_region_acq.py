...
logger = logging.getLogger("VISHESchedulingRegionACQ")


class VISHESchedulingRegionACQ(RegionACQ):
    def __init__(self):
        super().__init__()
        self.strategy = "VIS-HE-SCHEDULE"
        logger.debug("Initialized VISHESchedulingRegionACQ.")
    ...
    def select_regions(self, regions: List[Region], num_regions: int):
        ...
        return self._region_vis_he_scheduling(regions, num_regions)

    def _region_vis_he_scheduling(self, regions: List[Region], num_regions: int):
        num_to_select = min(num_regions, len(regions))
        logger.debug(
            f"Begin VIS-HE-SCHEDULING selection for {len(regions)} regions, number to select: {num_to_select}"
        )
        current_alpha = self.scheduler.get_value() if self.scheduler else self.alpha
        ...
        sampling_weights = (
            current_alpha * exploration_term + (1 - current_alpha) * exploitation_term
        )
