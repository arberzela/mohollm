...
logger = logging.getLogger("ScoreRegionACQAblations")


class ScoreRegionACQAblations(RegionACQ):
    def __init__(self):
        super().__init__()
        self.strategy = "ScoreRegionAblations"
        logger.debug("Initialized ScoreRegionACQAblations.")
    ...
    def select_regions(self, regions: List[Region], num_regions: int):
        ...
        ablation = self.space_partitioning_settings.get("ablation", None)
        if ablation not in [
            "exploration",
            "exploitation",
            "uniform_region_sampling",
            "ucb1",
        ]:
            raise ValueError(
                f"No valid ablation '{ablation}' was provided. Select from: ['exploration','exploitation','uniform_region_sampling','ucb1']"
            )
        ...
        for region in regions:
            ...
            if ablation == "ucb1":
                delta = 1.0 / max(
                    1, self.statistics.current_trial
                )  # Confidence parameter
                beta = np.sqrt(2 * np.log(1 / delta))
                ucb1 = beta / np.sqrt(n_l)
                ucb_bonus.append(ucb1)
            else:  # UCBV
        ...
        if ablation == "exploration":
            scores = beta_1 * volume_norm + beta_2 * moss_bonus_norm
        elif ablation == "exploitation":
            scores = exploitation_norm
        else:
        ...
        if ablation == "uniform_region_sampling":
            selected_indices = np.random.choice(
                len(regions), size=num_regions, replace=False
            )
        else:
            selected_indices = np.random.choice(
